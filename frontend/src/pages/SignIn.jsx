  ...
  return (
    <div className='w-full h-[100vh] bg-cover flex justify-center items-center' style={{ backgroundImage: `url(${bg})` }}>
      <form className='w-[90%] h-[600px] max-w-[500px] bg-[#00000069] backdrop-blur shadow-lg shadow-black flex flex-col justify-center items-center gap-[20px] px-[20px]' onSubmit={handleSignIn}>
        <h1 className='text-white text-[30px] font-semibold mb-[30px]'>SignIn To <span className='text-blue-500'>Virtual Assistant</span></h1>
        ...
        <button className='min-w-[150px] h-[60px] mt-[30px] text-black bg-white rounded-[10px] text-[18px] font-semibold cursor-pointer' disabled={loading}>{loading ? "Loading..." : 'Sign In'}</button>
        <p className='text-white text-[18px] cursor-pointer' onClick={() => navigate('/signup')}>Create a new account? <span className='text-blue-400'>Sign Up</span></p>
      </form>
    </div>
  )
}
